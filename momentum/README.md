...
- **Navigation (no tabs)**: one Home screen (`src/app/(main)/index.tsx`). Everything else opens on top of it and closes back:
  Focus as a full-screen modal (the floating button shows the time left), Insights from the progress ring, Settings and the
  research Library from the top bar (`components/SheetHeader.tsx`).
