...
### הדרך הקלה: סקריפט אוטומטי
בתיקייה `feynmanmind` מריצים:
- **Windows:** לחיצה כפולה על `setup-ai.bat`, או הקלדה של `setup-ai.bat` ב-cmd.
- **Mac/Linux:** `bash setup-ai.sh`

הסקריפט שואל את ה-Reference ID ואת מפתח Gemini, פותח דפדפן להתחברות ל-Supabase, שומר את המפתח בשרת ומעלה את ארבע הפונקציות. בסוף הוא מדפיס את שני הערכים שצריך להדביק באפליקציה. לא צריך Docker ולא צריך סיסמה למסד הנתונים.
